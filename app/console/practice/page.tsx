...
export default function PracticePage() {
  ...
  const [extractMeta, setExtractMeta] = useState<{ model: string } | null>(null);
  const [selectedKpIds, setSelectedKpIds] = useState<Set<string>>(new Set());

  // Persistence linkage
  const [persistedCourseId, setPersistedCourseId] = useState<string | null>(null);
  const [persistedKpMap, setPersistedKpMap] = useState<Record<string, string>>({});
    ...
    try {
      const res = await fetch("/api/generate-questions", {
        ...
        body: JSON.stringify({
          ...
          ...(persistedCourseId && Object.keys(persistedKpMap).length > 0
    ...
    <div className="mx-auto max-w-3xl space-y-6 p-6">
      ...
      <section className="space-y-4 rounded-xl border border-zinc-200 bg-white p-6 shadow-sm">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-lg font-semibold">上传课件 PDF</h2>
        </div>
        <form onSubmit={handleExtract} className="space-y-4">
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
            ...
            <div className="space-y-2 sm:col-span-2">
              ...
              <input
                ...
                className="w-full rounded-lg border border-zinc-300 bg-white px-3 py-2 file:mr-3 file:rounded file:border-0 file:bg-zinc-100 file:px-3 file:py-1"
              />
              {file && (
          ...
          <button
            ...
            className="w-full rounded-lg bg-zinc-900 px-4 py-2.5 font-medium text-white shadow-sm transition hover:bg-zinc-800 disabled:cursor-not-allowed disabled:opacity-50"
          >
      ...
      {outline && (
        <section className="space-y-4 rounded-xl border border-zinc-200 bg-white p-6 shadow-sm">
          <div className="flex items-center gap-2">
            ...
            />
            <h2 className="text-lg font-semibold">选择考点 + 出题设置</h2>
          </div>
          <div>
          ...
          <div className="space-y-3 border-t pt-4">
            <div className="flex items-center justify-between gap-3 rounded-lg border border-purple-200 bg-purple-50 px-3 py-2 text-xs">
          ...
          <div className="space-y-3 border-t pt-4">
            ...
            <p className="text-xs text-zinc-500">
              AI 按「必考 ≥ 重点 ≫ 了解」优先级 + 间隔重复 + 末段模考，给你一份逐日清单。
            </p>
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setDailyMinutes(
                      Math.max(15, Math.min(300, Number(e.target.value) || 60)),
                    )
                  }
                  className="mt-1 w-full rounded border border-zinc-300 bg-white px-2 py-1 text-sm"
                />
              ...
              <div className="flex items-end">
                <button
                  ...
                  className="w-full rounded-lg bg-emerald-700 px-4 py-2 text-sm font-medium text-white shadow-sm transition hover:bg-emerald-800 disabled:cursor-not-allowed disabled:opacity-50"
                >
                  {sprintLoading ? "生成中…" : "生成冲刺计划"}
                </button>
              </div>
            </div>
            {sprintError && <p className="text-sm text-red-600">⚠️ {sprintError}</p>}
      ...
      {questions.length > 0 && currentQ && (
        <section className="space-y-4 rounded-xl border border-zinc-200 bg-white p-6 shadow-sm">
          ...
          <div className="rounded-lg border border-zinc-200 bg-zinc-50 p-4 text-sm leading-relaxed">
          ...
          {currentQ.qtype !== "multiple_choice" && (
            <textarea
              ...
              rows={6}
              className="w-full rounded-lg border border-zinc-300 bg-white p-3 font-mono text-sm leading-relaxed focus:border-zinc-900 focus:outline-none disabled:cursor-not-allowed disabled:opacity-80"
            />
          )}
          ...
          {!currentGrade && (
            <button
              ...
              className="w-full rounded-lg bg-zinc-900 px-4 py-2.5 font-medium text-white shadow-sm transition hover:bg-zinc-800 disabled:cursor-not-allowed disabled:opacity-50"
            >
              {gradeLoading ? "批改中…" : "提交答案"}
            </button>
          )}
          {gradeError && <p className="text-sm text-red-600">⚠️ {gradeError}</p>}
...
// ============================================================================
// UI helpers (StepBadge / KpRow / GradePanel / SprintPlanPanel / MockExamReport)
// ============================================================================
function StepBadge({
  n,
  active,
...
function SprintPlanPanel({ plan }: { plan: SprintPlan }) {
  return (
    <div className="space-y-3 rounded-lg border border-emerald-200 bg-emerald-50/30 p-3 text-sm">
                ...
                {d.tasks.map((t, i) => (
                  ...
                  >
                    ...
                    <span className="flex-1">
                      ...
                      <span className="ml-1 text-zinc-400">({t.minutes}min)</span>
                      {t.note && (
...
function MockExamReport({
  ...
  return (
    <div className="space-y-4 rounded-lg border border-emerald-200 bg-emerald-50/60 p-4 text-sm">
      ...
      <div className="rounded bg-white p-3">
        <div className="text-xs font-medium text-zinc-700">📊 按考点等级分布</div>
      ...
      {topTags.length > 0 && (
        <div className="rounded bg-white p-3">
          <div className="text-xs font-medium text-zinc-700">🔥 高频错误类型</div>
      ...
      <p className="rounded bg-amber-50 px-3 py-2 text-xs text-amber-800">
        💡 建议先复盘错题（上 / 下一题），再回到 Step 2 重新选考点出新题或调整模拟卷。
      </p>
    </div>
  );
}
