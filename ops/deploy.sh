...
# --timeout 600: build can run 30-90s, needs > ssh_execute.py default 30s
python "$SSH_PY" "$ALIAS" "sudo -u linkao -H bash -lc 'cd /opt/linkao && git pull origin main && HUSKY=0 pnpm install --frozen-lockfile --ignore-scripts && HUSKY=0 NODE_ENV=production pnpm build 2>&1 | tail -8'" --timeout 600

echo ""
echo "→ 3/4  pm2 restart + sanity probe localhost:3001"
python "$SSH_PY" "$ALIAS" "sudo -u linkao -H bash -lc 'pm2 restart linkao && sleep 2 && pm2 list | tail -5 && curl -sI --max-time 10 http://127.0.0.1:3001/ | head -3'" --timeout 60
